...
if ($pedido == "login") {
    $lookup = $connection->prepare("SELECT * FROM users WHERE email=:email");
    ...
    */
    ...
    if (isset($arrayDados["email"])) {
        ...
        if ($verify == true) {
            $_SESSION["id_user"] = $arrayDados["id_user"];
...
if ($pedido == "registro") {
    ...
    if (isset($arrayDados["email"]) && $arrayDados["email"] == $inputEmail) {
        ...
        $sendData = $connection->prepare("INSERT users (email, password, nickname) VALUES (:email, :password, :nickname)");
        ...
        $bringData = $connection->prepare("SELECT * FROM users WHERE email=:email");
